...
(() => {
  ...
  function applyTransform() {
    ...
    canvasBg.style.setProperty('--bg-pos-x', `${tx}px`);
    canvasBg.style.setProperty('--bg-pos-y', `${ty}px`);
  ...
  function setupCardInteractions(card) {
    ...
    if (!el.querySelector('.card__actions')) {
      ...
      actions.innerHTML = `
        <button class="card-act-btn card-act-btn--upload" title="Upload / Change Image">
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none"><rect x="3.5" y="4.5" width="17" height="15" rx="2" stroke="currentColor" stroke-width="1.6"/><circle cx="8.5" cy="9.5" r="1.5" stroke="currentColor" stroke-width="1.4"/><path d="M4 16l5-4.5 3.5 3L17 10l3.5 4" stroke="currentColor" stroke-width="1.4" stroke-linejoin="round"/></svg>
        </button>
        <button class="card-act-btn card-act-btn--color" title="Cycle Accent Color">
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="9" stroke="currentColor" stroke-width="1.6"/><circle cx="12" cy="12" r="4" fill="currentColor"/></svg>
        </button>
        <button class="card-act-btn card-act-btn--duplicate" title="Duplicate Card">
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none"><rect x="9" y="9" width="13" height="13" rx="2" stroke="currentColor" stroke-width="1.6"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1" stroke="currentColor" stroke-width="1.6"/></svg>
        </button>
        <button class="card-act-btn card-act-btn--delete" title="Delete Card">
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none"><path d="M3 6h18M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2" stroke="currentColor" stroke-width="1.6" stroke-linecap="round"/></svg>
        </button>
      `;
    ...
    el.querySelectorAll('h3, p, li').forEach(editable => {
      editable.setAttribute('contenteditable', 'true');
      editable.addEventListener('focus', pushUndoState);
      editable.addEventListener('blur', saveToLocalStorage);
      ...
      contentHTML = `
        <div class="card__media">
          <svg viewBox="0 0 300 140" preserveAspectRatio="xMidYMid slice" xmlns="http://www.w3.org/2000/svg">
            ...
            <text x="50%" y="70%" dominant-baseline="middle" text-anchor="middle" fill="#B9B2A0" font-size="12">Click to Upload Image</text>
          </svg>
          <button class="btn-change-image" title="Upload / Change Image">📷 Change Image</button>
        </div>
        <div class="card__body">
          <h3>Media Node</h3>
          <p>Uploaded project visualization or diagram.</p>
        </div>
      `;
    } else if (type === 'note') {
      contentHTML = `
        <div class="card__head"><span class="card__icon">📌</span><h3>Quick Note</h3></div>
        <p>Click here to type your analysis notes and strategic observations...</p>
      `;
    } else if (type === 'shape') {
      contentHTML = `
        <div class="card__head"><span class="card__icon">⚡</span><h3>Key Component</h3></div>
        <ul>
          <li>Priority Task 1</li>
          <li>System Architecture</li>
          <li>Milestone Target</li>
        </ul>
      `;
    } else {
      contentHTML = `
        <div class="card__head"><span class="card__icon">💡</span><h3>New Analysis Node</h3></div>
        <p>Enter details, requirements, or architecture specifications for ZanketsuBoard here.</p>
      `;
    }
  ...
  function drawConnectors() {
    ...
    EDGES.forEach(([fromId, toId]) => {
      ...
      edgeLayer.appendChild(dotStart);
