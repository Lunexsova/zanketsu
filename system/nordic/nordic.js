    ...
    let categories = [
        {
            ...
            items: [
                { name: 'Cleansing the skin with organic extracts',  desc: 'Lavender oil + aloe vera',      price: '120' },
                { name: 'Natural green clay mask',      desc: 'Shrink pores + brighten',       price: '90' },
                { name: 'Thermal water moisturizing treatment',    desc: 'Suitable for sensitive skin',      price: '150' },
            ]
        },
        {
            name: 'Organic body care',
            items: [
                { name: 'Natural sugar and honey peeling',   desc: 'Complete body cell regeneration',  price: '130' },
                { name: 'Moisturizing whole shea butter',     desc: 'For the body + hands and feet',   price: '180' },
            ]
        }
    ];

    function renderBrochure() {
        const biz      = bizNameInput.value  || 'Nature Center for Organic Skin Care';
        const tagline  = taglineInput.value  || 'Pure nature · Radiant skin';
        const footer   = footerInput.value   || 'We celebrate your natural beauty · Limited reservations';
