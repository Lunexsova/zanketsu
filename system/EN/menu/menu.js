document.addEventListener('DOMContentLoaded', () => {
    ...
    let menuData = [
        {
            id: 1,
            name: 'Hair care (Hair Care)',
            items: [
                { id: 101, name: 'Classic haircut', price: '75', desc: 'Professional cutting, washing and styling with a blow dryer.' },
                { id: 102, name: 'Complete ammonia-free dye', price: '250', desc: 'Healthy and nourishing hair dye in modern colors.' }
            ]
        },
        {
            id: 2,
            name: 'skin care (Skin Care)',
            items: [
                { id: 201, name: 'Hydrafacial session for freshness', price: '350', desc: 'Deep skin cleansing with water peeling and vitamin nourishment.' }
            ]
        }
    ];
        ...
        menuData.forEach((cat, catIdx) => {
            ...
            catBlock.innerHTML = `
                <div class="category-header">
                    ...
                    <button type="button" class="delete-btn delete-cat-btn" title="Delete classification"><i class="ph-bold ph-trash"></i></button>
                </div>
                <div class="category-items-list"></div>
                <button type="button" class="add-btn-small add-item-btn"><i class="ph-bold ph-plus"></i> Add a service to this section</button>
            `;
            ...
            cat.items.forEach((item, itemIdx) => {
                ...
                itemRow.innerHTML = `
                    <div style="display: flex; flex-direction: column; gap: 0.2rem;">
                        <input type="text" class="form-control item-name-input" value="${item.name}" placeholder="Service name..." style="font-weight: 700;">
                        <input type="text" class="form-control item-desc-input" value="${item.desc}" placeholder="Simplified explanation..." style="font-size: 0.75rem;">
                    </div>
                    <input type="number" class="form-control item-price-input" value="${item.price}" placeholder="the price" style="text-align: center;">
                    <button type="button" class="delete-btn delete-item-btn" title="Delete the service"><i class="ph-bold ph-x"></i></button>
                `;
    ...
    function renderBrochure() {
        ...
        // Brand details
        prevBrandName.textContent = brandNameInput.value || 'Name of the center or salon';
        prevBrandSubtitle.textContent = brandSubtitleInput.value || 'Exclusive beauty packages and services';
