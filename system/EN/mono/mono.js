document.addEventListener('DOMContentLoaded', () => {
    ...
    let currentSkin = 'mono';
    let categories = [
        {
            name: '// HAIRCUT_SERVICES',
            items: [
                { name: 'قص_الشعر_رجالي',      desc: 'Classic Cut + Style',    price: '40' },
                { name: 'قص_الشعر_نسائي',      desc: 'Women Full Cut',         price: '70' },
                { name: 'صبغة_كاملة',          desc: 'Full Color Treatment',   price: '180' },
            ]
        },
        {
            name: '// BEARD_SERVICES',
            items: [
                { name: 'حلاقة_لحية_كاملة',    desc: 'Blade Shave + Balm',    price: '35' },
                { name: 'تشكيل_لحية',          desc: 'Beard Design + Wax',    price: '28' },
            ]
        }
    ];

    function renderBrochure() {
        const biz      = bizNameInput.value  || 'The Mono Barber Studio';
        const tagline  = taglineInput.value  || '// precision_cuts_since_2010.exe';
        const footer   = footerInput.value   || '> walk-ins_welcome | by_appointment_only';
        ...
        menuBrochure.innerHTML = `
            <div class="brochure-header">
                <div class="brochure-logo-wrap" id="brochureLogoWrap">
                    <div class="logo-placeholder"><i class="ph-duotone ph-terminal"></i></div>
                </div>
    ...
    categoryList.addEventListener('input', e => { const el = e.target; const ci = +el.dataset.ci, ii = +el.dataset.ii; if (el.classList.contains('category-name-input')) categories[ci].name = el.value; else if (el.classList.contains('item-name-input')) categories[ci].items[ii].name = el.value; else if (el.classList.contains('item-desc-input')) categories[ci].items[ii].desc = el.value; else if (el.classList.contains('item-price-input')) categories[ci].items[ii].price = el.value; renderBrochure(); });
    categoryList.addEventListener('click', e => { const btn = e.target.closest('button'); if (!btn) return; const ci = +btn.dataset.ci, ii = +btn.dataset.ii; if (btn.classList.contains('remove-cat')) { categories.splice(ci, 1); renderControls(); renderBrochure(); } else if (btn.classList.contains('remove-item')) { categories[ci].items.splice(ii, 1); renderControls(); renderBrochure(); } else if (btn.classList.contains('btn-add-item')) { categories[ci].items.push({ name: 'خدمة_جديدة', desc: '', price: '0' }); renderControls(); renderBrochure(); } });
    addCategoryBtn.addEventListener('click', () => { categories.push({ name: '// قسم_جديد', items: [{ name: 'خدمة_جديدة', desc: '', price: '0' }] }); renderControls(); renderBrochure(); });
