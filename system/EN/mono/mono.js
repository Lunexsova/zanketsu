document.addEventListener('DOMContentLoaded', () => {
    ...
    let categories = [
        {
            ...
            items: [
                { name: 'He told_the hair_My men',      desc: 'Classic Cut + Style',    price: '40' },
                { name: 'He told_the hair_female',      desc: 'Women Full Cut',         price: '70' },
                { name: 'tincture_Complete',          desc: 'Full Color Treatment',   price: '180' },
            ]
        },
        {
            name: '// BEARD_SERVICES',
            items: [
                { name: 'shaving_beard_Complete',    desc: 'Blade Shave + Balm',    price: '35' },
                { name: 'formation_beard',          desc: 'Beard Design + Wax',    price: '28' },
            ]
        }
    ];
    ...
    categoryList.addEventListener('click', e => { const btn = e.target.closest('button'); if (!btn) return; const ci = +btn.dataset.ci, ii = +btn.dataset.ii; if (btn.classList.contains('remove-cat')) { categories.splice(ci, 1); renderControls(); renderBrochure(); } else if (btn.classList.contains('remove-item')) { categories[ci].items.splice(ii, 1); renderControls(); renderBrochure(); } else if (btn.classList.contains('btn-add-item')) { categories[ci].items.push({ name: 'service_New', desc: '', price: '0' }); renderControls(); renderBrochure(); } });
    addCategoryBtn.addEventListener('click', () => { categories.push({ name: '// to divide_new', items: [{ name: 'service_New', desc: '', price: '0' }] }); renderControls(); renderBrochure(); });
