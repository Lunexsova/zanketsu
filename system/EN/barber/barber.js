document.addEventListener('DOMContentLoaded', () => {
    ...
    let categories = [
        {
            name: 'Men\'s haircut',
            items: [
                { name: 'Classic haircut',          desc: 'Cutting + shaping + moisturizing',          price: '45' },
                { name: 'Video cut and fade (Fade)',     desc: 'From the sides with a professional gradient',   price: '55' },
                { name: 'Cut + wash + dry',         desc: 'Includes shampoo and conditioner',      price: '65' },
            ]
        },
        {
            name: 'beard services',
            items: [
                { name: 'Shaving the beard with a razor',      desc: 'Classic mousse + pre-cream',  price: '40' },
                { name: 'Shaping and trimming the beard',      desc: 'Design according to the face',            price: '35' },
                { name: 'Beard oil treatment',       desc: 'Natural argan oil with massage',    price: '50' },
            ]
        },
        {
            name: 'Complete packages',
            items: [
                { name: 'Classic gentleman\'s bouquet',   desc: 'Cut + beard + full moisturizer',      price: '110' },
                { name: 'Groom\'s bouquet (VIP)',         desc: 'All services + perfume + mask',  price: '220' },
            ]
        }
    ];

    function renderBrochure() {
        const biz      = bizNameInput.value  || 'Al Basha Classic Barber Shop';
        const tagline  = taglineInput.value  || 'Classic shaving authenticity since 1985';
        const footer   = footerInput.value   || 'We welcome you · Working hours are 9am - 11pm';
    ...
    categoryList.addEventListener('click', e => {
        ...
        else if (btn.classList.contains('btn-add-item')) { categories[ci].items.push({ name: 'New service', desc: '', price: '0' }); renderControls(); renderBrochure(); }
    });
