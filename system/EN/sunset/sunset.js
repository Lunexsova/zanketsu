document.addEventListener('DOMContentLoaded', () => {
    ...
    let categories = [
        {
            name: 'Beauty and makeup services',
            items: [
                { name: 'Complete evening makeup',          desc: 'Makeup + lighting + 12 hour hold',  price: '250' },
                { name: 'Daily makeup (Soft Glam)',    desc: 'A bright and natural look',           price: '120' },
                { name: 'Bridal makeup (VIP)',         desc: 'Experience + Wedding Day + Touch-Up',    price: '600' },
            ]
        },
        {
            name: 'Skin care and tanning',
            items: [
                { name: 'Gradient bronze complexion',       desc: 'Self tanning without burning',           price: '180' },
                { name: 'Bellout (BB Glow Facial)',  desc: 'Semi-permanent makeup + freshness',         price: '350' },
            ]
        }
    ];

    function renderBrochure() {
        const biz      = bizNameInput.value  || 'Group beauty and makeup studio';
        const tagline  = taglineInput.value  || 'Where beauty meets sunset light';
        const footer   = footerInput.value   || 'Book your appointment now · Limited seats 🌅';
        const currency = currencySelect.value || 'SAR';
    ...
    function renderControls() {
        ...
        categories.forEach((cat, ci) => {
            ...
            catDiv.innerHTML = `
                ...
                <button class="btn-add-item" data-ci="${ci}"><i class="ph-bold ph-plus"></i> Add a service</button>`;
            ...
            cat.items.forEach((item, ii) => {
                ...
                row.innerHTML = `
                    ...
                    <input class="item-input item-desc-input" type="text" placeholder="${item.desc || 'Description (optional)'}" value="" data-ci="${ci}" data-ii="${ii}">
                    <div class="item-price-row">
                        <span class="item-price-label">the price:</span>
    ...
    categoryList.addEventListener('click', e => { const btn = e.target.closest('button'); if (!btn) return; const ci = +btn.dataset.ci, ii = +btn.dataset.ii; if (btn.classList.contains('remove-cat')) { categories.splice(ci, 1); renderControls(); renderBrochure(); } else if (btn.classList.contains('remove-item')) { categories[ci].items.splice(ii, 1); renderControls(); renderBrochure(); } else if (btn.classList.contains('btn-add-item')) { categories[ci].items.push({ name: 'New service', desc: '', price: '0' }); renderControls(); renderBrochure(); } });
    addCategoryBtn.addEventListener('click', () => { categories.push({ name: 'New section', items: [{ name: 'New service', desc: '', price: '0' }] }); renderControls(); renderBrochure(); });
