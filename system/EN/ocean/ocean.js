document.addEventListener('DOMContentLoaded', () => {
    ...
    let categories  = [
        {
            ...
            items: [
                { name: 'Full body massage (60 minutes)',   desc: 'Natural argan oil + Swedish technology', price: '180' },
                { name: 'Hot stone massage (90 minutes)',   desc: 'Natural marine basalt stones',  price: '250' },
                { name: 'Face and neck massage (30 minutes)',      desc: 'Removing facial muscle tension',       price: '120' },
            ]
        },
        {
            name: 'Skin care packages',
            items: [
                { name: 'Marine freshness bouquet',   desc: 'Exfoliating + moisturizing with deep extracts',    price: '320' },
                { name: 'Marine collagen treatment', desc: 'Lifting + re-stretching the skin',              price: '280' },
            ]
        }
    ];
    ...
    function renderBrochure() {
        const biz      = bizNameInput.value    || 'Laila Massage and Relaxation Center';
        const tagline  = taglineInput.value    || 'Where the sea and treatment meet in one place';
        const footer   = footerInput.value     || 'We are happy to serve you · Please book in advance';
    ...
    categoryList.addEventListener('input', e => {
        ...
        if (el.classList.contains('category-name-input')) {
            categories[ci].name = el.value || el.placeholder;
        } else if (el.classList.contains('item-name-input')) {
            categories[ci].items[ii].name = el.value || el.placeholder;
        } else if (el.classList.contains('item-desc-input')) {
            categories[ci].items[ii].desc = el.value || el.placeholder;
        } else if (el.classList.contains('item-price-input')) {
            categories[ci].items[ii].price = el.value || el.placeholder;
        }
        renderBrochure();
    });
    ...
    categoryList.addEventListener('click', e => {
        ...
        if (btn.classList.contains('remove-cat')) {
            ...
            categories[ci].items.push({ name: 'New service', desc: '', price: '0' });
            renderControls(); renderBrochure();
        }
    });
    ...
    addCategoryBtn.addEventListener('click', () => {
        categories.push({ name: 'New section', items: [{ name: 'New service', desc: '', price: '0' }] });
        renderControls(); renderBrochure();
    });
