document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateText() {
        previews.clinicName.textContent = inputs.clinicName.value || 'Clinic / Center Name';
        previews.docTitle.textContent = inputs.docTitle.value || 'Medical Consent & Acknowledgment Form';
        ...
        if (dobVal && !inputs.patientAge.value) {
            previews.patientAge.textContent = calcAge(dobVal) || '____';
        } else {
    ...
    function updateConditions() {
        ...
        conditionCheckboxes.forEach(cb => {
            if (cb.checked) {
                ...
                div.innerHTML = `
                    <span style="color: #dc2626; font-weight: bold; margin-right: 0.3rem;">✓</span>
                    <span>${label}</span>
                `;
