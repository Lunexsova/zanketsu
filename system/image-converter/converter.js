document.addEventListener("DOMContentLoaded", () => {
    ...
    async function handleFiles(files) {
        ...
        if (fileArray.length > MAX_FILES) {
            alert(`⚠️ الحد الأقصى المسموح به هو ${MAX_FILES} صور فقط في كل مرة.\nتم اختيار ${fileArray.length} صورة — يرجى تقليل العدد والمحاولة مجدداً.`);
            return;
        }
        ...
        if (oversized.length > 0) {
            ...
            alert(`⚠️ الصور التالية تتجاوز الحد الأقصى المسموح به (10 MB لكل صورة):\n\n${names}\n\nيرجى اختيار صور أصغر حجماً.`);
            return;
        }
        ...
        for (const file of fileArray) {
            loadedCount++;
            showUploadProgress(`جاري رفع الصورة ${loadedCount} من ${total}...`);
            ...
            await new Promise((resolve) => {
                ...
                reader.onload = function (e) {
                    ...
                    imgNode.onerror = function () {
                        console.error("فشل تحميل عنصر الصورة:", file.name);
                        resolve();
                    };
                    imgNode.src = e.target.result;
                };
                reader.onerror = function () {
                    console.error("خطأ قراءة الملف:", file.name);
                    resolve();
                };
    ...
    async function processAllImages(showProgress = false) {
        ...
        for (let idx = 0; idx < total; idx++) {
            ...
            if (showProgress) {
                showUploadProgress(`جاري رفع الصورة ${idx + 1} من ${total}...`);
            }
            ...
            await new Promise((resolve) => {
                canvas.toBlob((blob) => {
                    if (blob) {
                        ...
                        // Fallback if toBlob failed for format
    ...
    function renderResults() {
        ...
        if (imagesQueue.length === 1) {
            ...
            processedDims.textContent = `${item.targetWidth} x ${item.targetHeight} بكسل`;
                ...
                card.innerHTML = `
                    ...
                    <div class="multi-image-info">
                        ...
                        <div class="multi-image-stats">
                            ...
                            ${savingsPct > 0 ? `<span class="savings-pct">وفر ${savingsPct}%</span>` : ''}
                        </div>
                    </div>
                    <button class="multi-image-action" data-index="${index}" title="تحميل هذه الصورة">
    ...
    zipDownloadBtn.addEventListener("click", async () => {
        ...
        zipDownloadBtn.innerHTML = `<i class="ph-bold ph-spinner animate-spin"></i> جاري إنشاء ملف ZIP...`;
