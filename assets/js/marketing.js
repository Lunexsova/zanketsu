// ============================================================
// Zanketsu Marketing Script — Deferred, non-blocking
// ============================================================
...
if (partnerForm) {
    partnerForm.addEventListener('submit', function(e) {
        ...
        button.innerHTML = '<i class="ph-duotone ph-spinner-gap" style="animation: spin-glow 1s linear infinite; display: inline-block;"></i> جاري تقديم الطلب...';
        ...
        const payload = {
            ...
            template_params: {
                title: 'طلب انضمام جديد كشريك مؤسس (العربية)',
                name: nameVal,
                email: emailVal,
                social: socialVal,
                message: msgVal,
                new_customer: 'بوابة الشركاء'
            }
        };
        ...
        .then(async (response) => {
            if (response.status == 200) {
                showToast('تم تسجيل اهتمامك بنجاح! تم حجز مقعدك كشريك مؤسس وسنتواصل معك قريباً عبر البريد.');
                form.reset();
            } else {
                showToast('حدث خطأ أثناء الإرسال، يرجى المحاولة لاحقاً.');
            }
        })
        .catch(error => {
            showToast('حدث خطأ في الاتصال. يرجى التحقق من اتصالك بالإنترنت.');
