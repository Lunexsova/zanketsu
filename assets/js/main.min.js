// ============================================================
// Zanketsu Main Script — Deferred, non-blocking
// ============================================================
...
if (waitlistForm) {
    waitlistForm.addEventListener('submit', function(e) {
        ...
        button.innerHTML = '<i class="ph-duotone ph-spinner-gap" style="animation: spin-glow 1s linear infinite; display: inline-block;"></i> جاري الإرسال...';
        fetch('https://api.emailjs.com/api/v1.0/email/send', {
            ...
            body: JSON.stringify({
                ...
                template_params: {
                    title: 'تسجيل جديد في قائمة الانتظار (العربية)',
                    name: emailInput.value,
                    email: emailInput.value,
                    social: 'لا يوجد (تسجيل قائمة انتظار)',
                    message: 'تم تسجيل هذا البريد الإلكتروني في قائمة الانتظار.',
                    new_customer: 'بوابة Zanketsu'
                }
            })
        })
        .then(async (response) => {
            if (response.status === 200) { showToast('شكراً لتسجيلك! تم حفظ بريدك بنجاح وسنتواصل معك فور الإطلاق.'); form.reset(); }
            else { showToast('حدث خطأ أثناء التسجيل، يرجى المحاولة لاحقاً.'); }
        })
        .catch(() => showToast('حدث خطأ في الاتصال. يرجى التحقق من اتصالك بالإنترنت.'))
    ...
    if (introVideo) {
        function togglePlay() {
            if (introVideo.paused) {
                introVideo.play().then(() => {
                    if (videoPlayBtn) videoPlayBtn.innerHTML = '<i class="ph-fill ph-pause-circle"></i>';
                    if (videoOverlay) videoOverlay.classList.add('playing');
                });
            } else {
                introVideo.pause();
                if (videoPlayBtn) videoPlayBtn.innerHTML = '<i class="ph-fill ph-play-circle"></i>';
                if (videoOverlay) videoOverlay.classList.remove('playing');
            }
        }

        if (videoPlayBtn) videoPlayBtn.addEventListener('click', togglePlay);
        introVideo.addEventListener('click', togglePlay);
        ...
        if (videoMuteBtn) {
            videoMuteBtn.addEventListener('click', (e) => {
                ...
                if (introVideo.muted) {
                    videoMuteBtn.innerHTML = '<i class="ph-bold ph-speaker-slash"></i>';
                } else {
                    videoMuteBtn.innerHTML = '<i class="ph-bold ph-speaker-high"></i>';
                }
            });
        }
        ...
        // Intersection Observer to Autoplay (Muted) in Viewport, and Pause out of Viewport
        const videoViewObserver = new IntersectionObserver((entries) => {
            entries.forEach(entry => {
                if (entry.isIntersecting) {
                    introVideo.muted = true;
                    if (videoMuteBtn) videoMuteBtn.innerHTML = '<i class="ph-bold ph-speaker-slash"></i>';
        ...
        videoViewObserver.observe(introVideo);
    }
